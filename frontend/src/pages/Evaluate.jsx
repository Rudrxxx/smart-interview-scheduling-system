...
export default function Evaluate() {
  const { interviewId } = useParams()  // ← FIXED: was `id`, route uses `interviewId`
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate('/my-interviews')  // ← FIXED: redirect to interviewer route, not admin
    } catch (err) {
  ...
  return (
    <Layout>
      <div className="max-w-2xl mx-auto py-8">
        ...
        <p className="text-slate-400 text-sm mb-8">Interview #{interviewId}</p>

        <form onSubmit={handleSubmit} className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-6">
          {error && <div className="text-red-400 text-sm bg-red-900/20 border border-red-900/40 px-4 py-3 rounded-xl">{error}</div>}

          <div>
            <div className="flex justify-between mb-3">
              <label className="text-sm text-slate-400 font-medium">Candidate Score</label>
              <span className="text-white font-mono text-lg">{form.score}<span className="text-slate-500 text-sm">/100</span></span>
            </div>
            <input
          ...
          <div className="flex gap-3 pt-2">
            <button type="submit" disabled={saving}
              className="bg-white hover:bg-slate-100 text-slate-950 px-6 py-2.5 rounded-xl text-sm font-medium transition-all disabled:opacity-50">
